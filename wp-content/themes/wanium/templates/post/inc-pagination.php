...
$prev_date = $prev ? ( get_post_meta($prev->ID, '_tlg_portfolio_date', 1) ? date("M d, Y", get_post_meta($prev->ID, '_tlg_portfolio_date', 1)) : mysql2date('M d, Y', $prev->post_date, false) ) : false;
$next_date = $next ? ( get_post_meta($next->ID, '_tlg_portfolio_date', 1) ? date("M d, Y", get_post_meta($next->ID, '_tlg_portfolio_date', 1)) : mysql2date('M d, Y', $next->post_date, false) ) : false;
?>
